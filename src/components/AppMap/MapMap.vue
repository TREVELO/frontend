...
export default {
  ...
  methods: {
    ...
    markerClicked(index) {
      ...
      axiosInstance
        .post(`http://localhost/api/v1/attraction/favorite/view/${contentId}`)
        .then((res) => {
    ...
    fetchData() {
      ...
      if (subject != undefined && sentence != undefined) {
        axiosInstance
          .post(
            `http://localhost/api/v1/attraction/search?title=${sentence}&sidoCode=&gugunCode=&contentType=${subject}`
          )
          .then((res) => {
            ...
            for (let index = 0; index < this.attractions.length; index++) {
              this.markerPositions.push([
                ...
                this.attractions[index].contentId,
              ]);
            }
